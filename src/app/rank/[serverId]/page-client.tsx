...
export default function RankPage() {
  ...
  const [message, setMessage] = useState('=ÉˆÏ Checking your rank...');
  ...
  useEffect(() => {
    ...
    const checkRank = async () => {
      try {
        ...
        if (!discordId) {
          setMessage('G•Ó Discord ID is required to check your rank.');
          return;
        }
        ...
        setMessage(data.content || 'G•Ó Unable to fetch rank information.');
      } catch (error) {
        setMessage('G•Ó Error checking rank. Please try again.');
      }
    };
