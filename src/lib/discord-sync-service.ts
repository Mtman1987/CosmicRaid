...
class DiscordSyncService {
  ...
  async syncServerData(serverId: string): Promise<void> {
    try {
...
export async function syncServerData(serverId: string): Promise<void> {
  return discordSyncService.syncServerData(serverId);
}
