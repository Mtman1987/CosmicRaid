...
export default function RankPage() {
  ...
  const [message, setMessage] = useState('🔍 Checking your rank...');
  ...
  useEffect(() => {
    ...
    const checkRank = async () => {
      try {
        ...
        if (!discordId) {
          setMessage('❌ Discord ID is required to check your rank.');
          return;
        }
        ...
        setMessage(data.content || '❌ Unable to fetch rank information.');
      } catch (error) {
        setMessage('❌ Error checking rank. Please try again.');
      }
    };
